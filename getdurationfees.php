...
if($_POST['subcourse'] && $_POST['subcourse']!="")
{
   ...
   $sel=mysqli_query($db,"SELECT subcourse,fees,duration FROM sub_course WHERE FIND_IN_SET(sub_course.id,'".$subcourse."')");
   ...
   if(mysqli_num_rows($sel) >0)
   {
      ...
      while( $res = mysqli_fetch_array($sel)){
         $divs=$divs. '<option value="'.$res['duration'].'">'.$res['duration'].'</option>';
